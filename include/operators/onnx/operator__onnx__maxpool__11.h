...
# ifndef OPERATOR_OPERATOR__ONNX__MAXPOOL__11_H
# define OPERATOR_OPERATOR__ONNX__MAXPOOL__11_H

# include "operators/operator.h"
# include "operators/operator_stub.h"

typedef struct operator_context_input__operator__onnx__maxpool__11 {
    size_t length;
    operator_tensor *X;
} operator_context_input_operator__onnx__maxpool__11;

typedef struct operator_context_output__operator__onnx__maxpool__11 {
    size_t length;
    operator_tensor *Y;
operator_tensor *Indices;
} operator_context_output_operator__onnx__maxpool__11;

typedef struct operator_context_attribute__operator__onnx__maxpool__11 {
    size_t length;
...
} operator_context_attribute_operator__onnx__maxpool__11;

typedef struct operator_context__operator__onnx__maxpool__11 {
    struct operator_context_input__operator__onnx__maxpool__11     *input;
    struct operator_context_output__operator__onnx__maxpool__11    *output;
    struct operator_context_attribute__operator__onnx__maxpool__11 *attribute;
    operator_executer                          operator;
} operator_context__operator__onnx__maxpool__11;

/**
 * onnx operator 'MaxPool' version 11
 *
 * @param[in]  ctx  Operator context
 * @return          Status code
 *
 * MaxPool consumes an input tensor X and applies max pooling across the
 * tensor according to kernel sizes, stride sizes, and pad lengths. max pooling
 * consisting of computing the max on all values of a subset of the input
 * tensor according to the kernel size and downsampling the data into the
 * output tensor Y for further processing. The output spatial shape will be
 * following: ``` output_spatial_shape[i] = floor((input_spatial_shape[i] +
 * pad_shape[i] - ((kernel_spatial_shape[i] - 1) * dilations[i] + 1)) /
 * strides_spatial_shape[i] + 1) ``` or ``` output_spatial_shape[i] =
 * ceil((input_spatial_shape[i] + pad_shape[i] - ((kernel_spatial_shape[i] - 1)
 * * dilations[i] + 1)) / strides_spatial_shape[i] + 1) ``` if ceil_mode is
 * enabled
 * ``` * pad_shape[i] is sum of pads along axis i ```
 * `auto_pad` is a DEPRECATED attribute. If you are using them currently, the
 * output spatial shape will be following: ``` VALID: output_spatial_shape[i] =
 * ceil((input_spatial_shape[i] - ((kernel_spatial_shape[i] - 1) * dilations[i]
 * + 1) + 1) / strides_spatial_shape[i]) SAME_UPPER or SAME_LOWER:
 * output_spatial_shape[i] = ceil(input_spatial_shape[i] /
 * strides_spatial_shape[i]) ``` And pad shape will be following if
 * `SAME_UPPER` or `SAME_LOWER`: ``` pad_shape[i] = (output_spatial_shape[i] -
 * 1) * strides_spatial_shape[i] + ((kernel_spatial_shape[i] - 1) *
 * dilations[i] + 1) - input_spatial_shape[i] ``` The output of each pooling
 * window is maximum number of elements exclude pad.
 * 
 * Constraint T:
 ...
 * @since version 11
 *
 * @see ../../../../../../travis/build/onnx/wheel-builder/onnx/onnx/defs/nn/defs.cc:342
 * @see https://github.com/onnx/onnx/blob/master/docs/Operators.md#MaxPool
 */
operator_status operator__onnx__maxpool__11(
...
operator_status operator__onnx__maxpool__11__T_tensor_double__I_tensor_int64(
    operator_context__operator__onnx__maxpool__11 *ctx
);
extern __attribute__((weak))
operator_status operator__onnx__maxpool__11__T_tensor_float__I_tensor_int64(
    operator_context__operator__onnx__maxpool__11 *ctx
);
extern __attribute__((weak))
operator_status operator__onnx__maxpool__11__T_tensor_float16__I_tensor_int64(
    operator_context__operator__onnx__maxpool__11 *ctx
);
# endif
