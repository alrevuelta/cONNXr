...
def copy_files(domain, opset, test_name, test_folder, out_folder):
    ...
    try:
        ...
        print("copying", test_folder, output_path)
    except FileExistsError:
        print("failed", test_folder, output_path)
            ...
            for test in os.listdir(node_path):
                if test != "__init__.py":                    
                    ...
                    if not operator:
                        ...
                        domain = [i.domain for i in ALL_SCHEMAS if i.name == operator[0]]
                    ...
                    mappings = {"ai.onnx": version[2],
                                ...
                                "ai.onnx.preview.training": version[4] if len(version) > 4 else '-1', # this field is optional
                                "custom": 1}

                    os.makedirs(os.path.join(args.casepath, domain), exist_ok=True)
                    os.makedirs(os.path.join(args.casepath, domain, str(mappings[domain])), exist_ok=True)
                    shutil.copy(case_full_path, os.path.join(args.casepath, domain, str(mappings[domain])))
