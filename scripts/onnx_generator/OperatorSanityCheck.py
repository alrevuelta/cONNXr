...
'''
    ...
    {{ // check if attributes have valid types
        check_operator_condition_attribute conditions[{n_conditions}] = {{
            {conditions}
        }};
        valid &= check_operator_attributes("{operator_name}",
                                           {n_conditions},
                                           conditions,
                                           attribute);
    }}
    ...
    def text(self,prefix=""):
        ...
        for attribute in self.schema.attributes:
            conditions.append(self._template_condition.format(
                ...
                type = attribute.onnxAttributeDataType()
            ).strip())
...
{{ // check if {tensors} tensors have valid types
    {types}
    check_operator_condition_tensor conditions[{n_conditions}] = {{
        {conditions}
    }};
    valid &= check_operator_tensors("{operator_name}",
                                     {n_conditions},
                                     conditions,
                                     {tensors});
}}
...
{{ // check if argument number is in valid range
    check_operator_condition_range condition_input = {{
        .name = "input",
        .min  = {min_input},
        .max  = {max_input}
    }};
    check_operator_condition_range condition_output = {{
        .name = "output",
        .min  = {min_output},
        .max  = {max_output}
    }};
    valid &= check_operator_constraint({operator_name},
                                       &condition_input,
                                       n_input);
    valid &= check_operator_constraint({operator_name},
                                       &condition_output,
...
{{ // check if multiple tensors constrained by '{constraint}' have same type
    check_operator_condition_constraint conditions_input[{n_conditions_input}] = {{
        {conditions_input}
    }};
    check_operator_condition_constraint conditions_output[{n_conditions_output}] = {{
        {conditions_output}
    }};
    valid &= check_operator_constraint("{operator_name} {constraint}",
                                       {n_conditions_input},
                                       conditions_input,
                                       input,
                                       {n_conditions_output},
...
'''
    ...
    def filename(self):
        ...
        path += f"/check_{self.schema.operator_name}.c"
