...
for h in headers:
  filename = f"{h.schema.operator_name}.h"
  dirname = f"{args.path[0]}/{args.header[0]}/{h.schema.domain}/"
  if args.verbose:
    print(f"writing header {dirname}/{filename}")
    os.makedirs(dirname,exist_ok=True)
    open(f"{dirname}/{filename}","w").write(h.text())
