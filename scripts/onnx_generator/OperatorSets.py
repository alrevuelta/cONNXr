...
'''
    def __init__(self, domain, name, schemas, path):
        ...
        for s in schemas:
            entries.append(f"""
#ifdef CONFIG_HAVE_{s.operator_name.upper()}
        &opversion_{s.operator_name},
#endif""")
            extern.append(f'extern operator_set_opversion opversion_{s.operator_name};')
        self.entries = "".join(entries).strip()
        self.extern = "\n".join(extern)
...
'''
    def __init__(self, domain, operators, path):
        ...
        for op in operators:
            ...
            entries.append(f"""
#ifdef CONFIG_HAVE_OPERATOR__{self._domain.upper()}__{op.upper()}
        &opname_operator__{self._domain}__{_op},
#endif""")
...
'''
    ...
    def __init__(self, domains, path):
        ...
        for domain in domains:
            ...
            entries.append(f"""
#ifdef CONFIG_HAVE_DOMAIN__{_domain.upper()}
        &opdomain_operator__{_domain},
#endif""")
